...
func main() {
  ...
  r.Path("/events_by_chapter/{id}").Methods(http.MethodGet, http.MethodOptions).HandlerFunc(get_all_events_by_chapter)
  r.Path("/spanSearch").Queries("trace_id", "{trace_id:[a-zA-Z0-9_]*?}", "user_id", "{user_id:[a-zA-Z0-9_]*?}", "session_id", "{session_id:[a-zA-Z0-9_]*?}", "chapter_id", "{chapter_id:[a-zA-Z0-9_]*?}", "status_code", "{status_code:[0-9]*?}").HandlerFunc(span_search_handler)
