...
func main() {
  ...
  // .Queries("session_id", "{[a-zA-Z0-9_]*?}").Queries("chapter_id", "{[a-zA-Z0-9_]*?}")
  ...
  r.Path("/events_by_chapter/{id}").Methods(http.MethodGet, http.MethodOptions).HandlerFunc(get_all_events_by_chapter)
  r.Path("/spanSearch").Queries("trace_id", "{[a-zA-Z0-9_]*?}").Queries("user_id", "{[a-zA-Z0-9_]*?}").Queries("status_code", "{[0-9]*?}").HandlerFunc(span_search_handler)
